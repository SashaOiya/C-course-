...
$
        for ( size_t i = 0, elem_number = elements_number; i < elem_number; ++i ) {
            T new_element = 0;
            std::cin >> new_element;
            std::deque<int> *element_pointer = find_elem_hash_table ( new_element );
...
$
    ...
    {
        size_t misses_total_n = 0;//= filling_cache (); 
$
        size_t elem_n = data_vector.size();
        for ( size_t i = 0; ( i < elem_n ) && ( data_vector.size() ); ++i ) {
            // get next elem
            T element = data_vector.front();  // list may be
            data_vector.pop_front();
$
            // get irr
            std::deque<int> *element_vector_pointer = find_elem_hash_table ( element ); 
