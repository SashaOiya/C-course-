...
private :
    ...
    std::vector<T> cache_list = {}; // name
                      // data, irr
    std::unordered_map< T, int> cache_hash_table = {}; // compulsory
...
$           element_pointer->push_back ( i );
    ...
    {
        size_t start_size = cache.cache_list.size(); // optim
...
$
    ...
    bool compare ( const std::pair<T, int> &a, const std::pair<T, int> &b )
    {
        return a.second < b.second;
    }

    inline int cache_processing ( Cache_c<T> &cache )
    {
        size_t misses_total_n = cache.filling_cache ( cache ); 
$
        size_t elem_n = cache.data_vector.size();
        int max_irr_element = 0;
